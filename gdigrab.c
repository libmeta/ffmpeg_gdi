...
static DwmIsCompositionEnabledFunc dwmIsCompositionEnabled = NULL;
static HMODULE dwmapi_library_  = NULL;
/**
...
#define CURSOR_ERROR(str)                 \
    ...
    if (GetCursorInfo(&ci)) {
        ...
        if (pos.x >= 0 && pos.x <= clip_rect.right - clip_rect.left &&
            ...
            if (!DrawIconEx(gdigrab->dest_hdc, pos.x, pos.y, icon,0,0,0,NULL,DI_NORMAL | DI_COMPAT))
...
static int gdigrab_read_packet(AVFormatContext *s1, AVPacket *pkt)
{
    ...
    BOOL result = FALSE;
    BOOL iswin = (gdigrab->hwnd != NULL)&&(IsWindow(gdigrab->hwnd));
    if (iswin)
    {
        BOOL isWindows80rLater = FALSE;
        int major, minor;
        ...
        if (GetVersionEx(&info))
        {
            ...
            isWindows80rLater =  (major > 6 || (major == 6 && minor >= 2));
        }

        if (isWindows80rLater) {
            const UINT pwRenderFullContent=0x00000002;
            const UINT pwClientOnly = 0x00000001;
            result = PrintWindow(gdigrab->hwnd, dest_hdc,pwRenderFullContent|pwClientOnly);
        }
    }
    ...
    if (iswin)
    {
        if (!result)
        {
            ...
            if (!isAeroEnabled)
            {
                result = PrintWindow(gdigrab->hwnd, dest_hdc, 0);
            }
        }
    }
    ...
    if (!result)
    {
        uint64_t flag = SRCCOPY;
        if (gdigrab->captureblt == 1)
        {
            flag |= CAPTUREBLT;
        }
...
static const AVOption options[] = {
    ...
    { "captureblt", "captureblt,0 is disable,1 is enable", OFFSET(captureblt), AV_OPT_TYPE_INT, {.i64 = 1}, 0, 1, DEC },
    { NULL },
};
